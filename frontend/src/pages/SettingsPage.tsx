...
export function SettingsPage() {
  ...
  return <Stack component="form" onSubmit={save} gap={3} sx={{ maxWidth: 850 }}><Box><Typography variant="h3">Configurações</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>Os detalhes que tornam esse dia só de vocês.</Typography></Box>
    ...
    <Card><CardContent sx={{ p: { xs: 2.5, sm: 3.5 } }}><Typography variant="h5" sx={{ mb: 2 }}>Confirmação de presença</Typography><FormControlLabel control={<Switch checked={form.acompanhantes_habilitados} onChange={e => field('acompanhantes_habilitados', e.target.checked)} />} label="Permitir acompanhantes" /><Typography variant="body2" color="text.secondary">Os convidados poderão informar quantas pessoas irão com eles (até 30). Ao desabilitar, as quantidades de acompanhantes registradas serão zeradas.</Typography></CardContent></Card>
