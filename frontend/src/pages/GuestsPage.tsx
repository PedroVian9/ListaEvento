...
import type { ConvidadoPor, EventInfo, Guest, MemberInput, Status } from '../types'
import { invitationLink } from '../invitationLink'

type GuestInput = Pick<Guest, 'nome' | 'observacao' | 'status_presenca' | 'quantidade_acompanhantes' | 'convidado_por'> & { membros: MemberInput[] }
const blank: GuestInput = { nome: '', observacao: '', status_presenca: 'PENDENTE', quantidade_acompanhantes: 0, convidado_por: 'AMBOS', membros: [] }
const sourceLabels: Record<ConvidadoPor, string> = { PEDRO: 'Pedro', MARIA: 'Maria', AMBOS: 'Dos dois' }
export function GuestsPage() {
  ...
  function openForm(guest?: Guest) {
    setEditing(guest || 'new'); setForm(guest ? { nome: guest.nome, observacao: guest.observacao, status_presenca: guest.status_presenca, quantidade_acompanhantes: guest.quantidade_acompanhantes, convidado_por: guest.convidado_por, membros: guest.convite_familiar ? guest.membros.map(m => ({ ...m })) : [] } : { ...blank, membros: [] }); setFormError(''); setMenu(null)
  }
